...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!commands.isEmpty() && !_commands.isEmpty()) {
            throw new IllegalStateException("error: Cannot add now.");
        } else {
    ...
    private void checkFinishedDependencies() {
        Iteration<Integer> vertSucc = _depends.successors(_vertex);
        while (vertSucc.hasNext()) {
            if (_depends.getLabel(vertSucc.next()).isUnfinished()) {
                error("error: Dependency not built!");
            }
        }
    ...
    private boolean outOfDate() {
        if (_time == null) {
            ...
            Iteration<Integer> vertSucc = _depends.successors(_vertex);
            while (vertSucc.hasNext()) {
                if (_time < _depends.getLabel(vertSucc.next()).getTime()) {
                    return true;
                }
            }
        }
